...
class CourseDetailScreen extends StatefulWidget {
  final CourseMaster course;
  final String slug; // 🔹 NEW: slug bhi aa raha hai
...
class _CourseDetailScreenState extends State<CourseDetailScreen>
  ...
  // 🔹 Single course controller (API driven)
  final SingleCourseController singleCourseController = Get.find();
  ...
  void initState() {
    ...
    LogX.printLog("🧵 Initial Course is_enroll (model): ${widget.course.is_enroll}");
    LogX.printLog("🎯 Incoming slug for this screen: ${widget.slug}");

    // 🔹 Post-frame callback to safely call API
    WidgetsBinding.instance.addPostFrameCallback((_) {
      LogX.printLog(
          "📡 Calling SingleCourseController.fetchCourseBySlug with slug: ${widget.slug}");
      singleCourseController.fetchCourseBySlug(widget.slug);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        // 🔹 Pehle controller se API course lo, fallback: widget.course
        final CourseMaster course =
            singleCourseController.singleCourse.value ?? widget.course;

        LogX.printLog(
            "📦 Rendering CourseDetailScreen for course: ${course.courseTitle}, is_enroll: ${course.is_enroll}");

        final rawCurriculum = course.courseDetail?.curriculum ?? '';
        final curriculumItems = splitCurriculumByNumbers(rawCurriculum);
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(height: isTablet ? 24 : 18),
                                      Text(
                                        parseHtmlString(course.courseDetail
                                            ?.whatYouLearn ??
                                            'Nothing'),
                                        style: GoogleFonts.lato(
                                          fontSize: isTablet ? 16 : 14,
      ...
      bottomNavigationBar: FutureBuilder<bool>(
        ...
        builder: (context, snapshot) {
          ...
          // 🔹 Neeche bhi API se updated course use karo
          final CourseMaster course =
              singleCourseController.singleCourse.value ?? widget.course;
          ...
          return
            ...
            child: isLoggedIn
                  ...
                  return Expanded(
                    ...
                    ElevatedButton(
                      onPressed: isBusy
                        ...
                        try {
                          final userId =
                          StorageService.getData("User_id");
                          await addToCart.addToCart(
                            user_id: userId,
                            course_id: course.id,
                            quantity: 1,
              ...
              child: Text(
                "Enroll Course (₹${course.courseFee.toStringAsFixed(0)})",
                style: GoogleFonts.lato(
                  fontSize: isTablet ? 16 : 14,
                  color: Colors.white,
                ),
